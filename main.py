...
# 1. Telegram 推播設定 (優先讀取環境變數，未設定則使用預設值)
BOT_TOKEN = os.getenv(
    "TELEGRAM_BOT_TOKEN", "8321686170:AAGqyD9TSLBtw3B5XV0Z9r8ShMEgiYidv-E"
)
CHAT_ID = os.getenv("TELEGRAM_CHAT_ID", "您的_CHAT_ID")  # 👈 請填入您的 Chat ID
...
def run_strategy(target_input):
    ...
    if target_input.startswith("^"):
        ...
        ticker = target_input
        stock_name = "台灣加權指數"
    elif target_input in ["2330", "0050"]:
        stock_id = target_input
        ticker = f"{target_input}.TW"
        stock_name = "台積電" if target_input == "2330" else "元大台灣50"
    else:
        stock_id = target_input
        ticker = (
            f"{target_input}.TW"
    ...
    if stock_id == "^TWII":
        ...
        take_profit, stop_loss, max_hold_days = 0.035, -0.02, 7
    elif stock_id == "2330":
    ...
    msg = f"📌 <b>【{stock_name} ({stock_id}) 最新狀態】</b>\n"
...
if __name__ == "__main__":
    targets = ["2330", "^TWII", "0050"]
    full_report = "📊 <b>每日 AI 股票定量策略報告</b>\n\n"

    for t in targets:
        full_report += run_strategy(t) + "\n" + "=" * 25 + "\n\n"
